...
rsa_decrypt(wchar_t * filename, char* passwd) {
	...
	/*
	const wchar_t * priv_key = L"private.pem";
	// 用来存放密文
	char encrypted[1024];
	...
	FILE* priv_fp;
	if ((file = _Py_wfopen((char*)priv_key, L"rb")) == NULL) {
		fprintf(stdout,"failed to open priv_key file !\n");
		return -1;
	}
	...
	if (rsa2 == NULL) {
		fprintf(stdout, "unable to read private key!\n");
		return -1;
	}

	// 用私钥解密
	int len = RSA_private_decrypt(len, encrypted, decrypted, rsa2, RSA_PKCS1_PADDING);
	if (len == -1) {
		fprintf(stdout, "failed to decrypt!\n");
		return -1;
	}
	fclose(priv_fp);
	...
	*/
	...
	if (aes_len < 0)
	    ...
	    fprintf(stdout, "RSA_private_decrypt 123456789 \n");
                     ...
                     Py_ssize_t *opt_index)
{
    ...
    do {
		...
		/*添加的命令行参数k*/
		if (c == 'k') {
			fprintf(stdout, "using -k to add aes_key\n");
			size_t len = wcslen(_PyOS_optarg);
			keyfilename = malloc(sizeof(wchar_t) * (len+1));
			if (keyfilename == NULL)
				Py_FatalError(
					"not enough memory to copy -k argument");
			wcscpy(keyfilename, _PyOS_optarg);
			fprintf(stdout, "use aes_key success : %ls\n", keyfilename);
			keyfilename[len] = '\0';
			break;
		}
		/*读取完毕*/
	...
	/*对key文件进行解密处理*/
	if (keyfilename != NULL) {
		fprintf(stdout, "decrypting aes_key : %ls\n", keyfilename);
		...
		/*
		if (passwdlen < 0) {
			fprintf(stderr, "parsing key file '%ls' error\n" , keyfilename);
			free(keyfilename);
			return _PyStatus_EXIT(2);
		}
		else {
...
rsa_decrypt(wchar_t* filename, char* passwd) {
	...
	if (aes_len < 0)
		...
		set_aes_passwd(decrypted_aes_key);
		fprintf(stdout, "RSA_private_decrypt %s\n", decrypted_aes_key);
	}
