...
/*aes解密函数*/
int aes_decrypt(wchar_t * filename, const char* aes_passwd,char* plaintext) {
	return 0;
}
...
/* 以解密方式打开文件 */
FILE*
decrypt_open(const wchar_t* filename, const char* mode)
{
	int plainlen = -1;
	char* plaintext = NULL;
	FILE* fp = NULL;
...
FILE*
decrypt_reopen(const char* filename)
{
	int plainlen = -1;
	char* plaintext = NULL;
	FILE* fp = NULL;
...
pymain_run_file(PyConfig *config, PyCompilerFlags *cf)
{
	...
	FILE* fp = decrypt_open(filename, L"rb");
    if (fp == NULL) {
